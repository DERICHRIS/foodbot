...
# ---------------- STEP 5: LLM-BASED NUTRITION SUMMARY ---------------- #

st.subheader("📝 AI Summary of Your Day's Intake")

@st.cache_resource
def load_summary_generator():
    return pipeline("text-generation", model="distilgpt2")

gen = load_summary_generator()

def generate_summary(nutrient_totals):
    text = f"""
    Today your diet included approximately:
    - {nutrient_totals[0]:.0f} kcal
    - {nutrient_totals[1]:.1f} g protein
    - {nutrient_totals[2]:.1f} g carbohydrates
    - {nutrient_totals[3]:.1f} g fats
    - {nutrient_totals[4]:.1f} g sugar
    - {nutrient_totals[5]:.1f} g fiber
    - {nutrient_totals[6]:.1f} mg Vitamin C
    - {nutrient_totals[7]:.1f} mg Iron
    - {nutrient_totals[8]:.1f} mg Calcium

    Please summarize this nutrition data for the user in friendly language:
    """

    result = gen(text, max_length=200, do_sample=True, temperature=0.7)[0]['generated_text']
    return result[len(text):]  # Only return generated part

if total_nutrients.sum() > 0:
    summary = generate_summary(total_nutrients)
    st.markdown("### 🧾 AI Summary")
    st.success(summary.strip())
else:
    st.info("Log meals to generate a summary.")
